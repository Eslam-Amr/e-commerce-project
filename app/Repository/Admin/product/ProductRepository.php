...
class ProductRepository implements ProductRepositoryInterface
{
    ...
    public function index()
    {
        // dd(auth("admin")->user());
        if (auth('admin')->check()) {
            $products = Product::where('admin-acceptance',1)->where('status','accepted')->paginate();
        }
        // Check if the user is authenticated as seller
        else if (auth('seller')->check()) {
            $products = Product::where('admin-acceptance',1)->where('status','accepted')->where('seller_id', auth('seller')->user()->id)->paginate();
        }
